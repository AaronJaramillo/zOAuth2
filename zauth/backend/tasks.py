...
def process_tx(tx, product):
    transaction, created = Transaction.objects.select_for_update().get_or_create(txid=['txid'], address=tx['address'], amount=int(tx['amount']*(10**8)), memo=tx['memo'])
